...
conda run -n meeko_env --no-capture-output mk_prepare_receptor.py -i ./datas/1iep_receptorH.pdb -o 1iep_receptor -p -v \
...
conda run -n meeko_env --no-capture-output mk_prepare_ligand.py -i ./datas/1iep_ligand.sdf -o 1iep_ligand.pdbqt;

# Exécution du docking avec AutoDock Vina
# Utilise le fichier de configuration généré lors de la préparation du récepteur
conda run -n vina_env --no-capture-output python3 launch_vina.py --sf_name vina --receptor ./1iep_receptor.pdbqt --ligand ./1iep_ligand.pdbqt \
